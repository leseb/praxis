...
| [response-store.yaml](configs/ai/openai/responses/response-store.yaml) | Persists non-streaming Responses API responses to a SQLite database |
...
| [tcp-timeouts.yaml](configs/protocols/tcp-timeouts.yaml) | TCP proxy with idle and max duration timeouts. `tcp_idle_timeout_ms` closes connections that have been idle (no data in either direction) longer than the threshold. `tcp_max_duration_secs` caps the total session duration regardless of activity |
