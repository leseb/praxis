...
/// Backend response for the second turn — returned after rehydration.
const SECOND_RESPONSE_JSON: &str = r#"{"id":"resp_second","created_at":2000,"model":"gpt-4.1","object":"response","status":"completed","input":"What next?","output":[{"type":"message","content":[{"type":"output_text","text":"Let me help"}]}]}"#;
...
async fn rehydrate_enriches_second_request_with_conversation_history() {
    ...
    assert_eq!(
        status2, 200,
        "second request with previous_response_id should succeed (rehydration worked), body: {body2}"
    );
