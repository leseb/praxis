...
//! Functional integration tests for the tcp-active-connections example
//! configuration.
...
#[test]
fn tcp_active_connections_example_emits_gauge() {
    let backend_port = start_tcp_tagged_backend("pg");
    let proxy_port = free_port();
    let admin_port = free_port();
    let config = super::load_example_config(
        "observability/tcp-active-connections.yaml",
        proxy_port,
        HashMap::from([
...
#[test]
fn tcp_active_connections_example_forwards_traffic() {
    ...
    assert!(
        ...
        "tcp-active-connections example should forward to tagged backend, got: {resp}"
    );
}
