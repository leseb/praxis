...
fn store_adapted_request_body(ctx: &mut PingoraRequestCtx, body: Option<Bytes>) {
...
mod tests {
    ...
    /// Build a pipeline whose merged request-body mode is `StreamBuffer` with the
    /// given max, via a participant filter that declares it.
    ...
    fn selected_upstream_body_limit_uses_stream_buffer_max() {
        // A StreamBuffer pipeline reports its merged max as the effective limit.
    ...
    fn selected_upstream_body_limit_falls_back_to_absolute_max() {
        // An empty pipeline has no StreamBuffer bound; the absolute ceiling applies.
    ...
    fn selected_upstream_body_limit_honors_tighter_listener_ceiling() {
        // A listener body ceiling tighter than the filter's StreamBuffer max must
        // win: the adapted output is checked against the effective (clamped) bound,
        // never the individual filter's larger declared limit. This exercises the
        // `min(ceiling)` branch that the two tests above never reach.
