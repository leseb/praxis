...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn on_request_initializes_store_for_get_method() {
    let filter = make_filter();
    ...
    assert!(
        matches!(action, FilterAction::Continue),
        "should continue for GET method"
    );
    assert!(
        filter.store.get().is_some(),
        "store should be eagerly initialized for GET requests"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn on_request_initializes_store_for_delete_method() {
    let filter = make_filter();
    ...
    assert!(
        matches!(action, FilterAction::Continue),
        "should continue for DELETE method"
    );
    assert!(
        filter.store.get().is_some(),
        "store should be eagerly initialized for DELETE requests"
    );
}
